	...
	private void DrawWayangKulit(Vector2 position, float scale)
{
	...
	// Calculate arm angles with animation
	...
	// Create a transform matrix for the whole puppet (simplified to rotation only here)
	Vector2 rotateOrigin = position + new Vector2(0, 50) * scale; // Pivot point near the center
	
	// Draw components in proper wayang kulit order (back to front)
	DrawWayangKulitHandleRod(position, scale);  // Central rod stays fixed
	
	// Apply rotation and subtle scaling to the body parts
...
private Vector2 RotatePoint(Vector2 point, Vector2 origin, float angle)
{
	...
	Vector2 rotated = new Vector2(
		translated.X * Mathf.Cos(angle) - translated.Y * Mathf.Sin(angle),
		translated.X * Mathf.Sin(angle) + translated.Y * Mathf.Cos(angle)
	);
...
private void DrawWayangKulitAnimatedArms(Vector2 position, float scale, float leftArmAngle, float rightArmAngle)
{
	...
	Vector2 leftHandPos = leftElbowPos + new Vector2(
		leftHandLength * Mathf.Cos(leftRadians * 1.1f), // Slightly more bent
		leftHandLength * Mathf.Sin(leftRadians * 1.1f)
	);
	
	// Right arm with animated angle
	...
	Vector2 rightHandPos = rightElbowPos + new Vector2(
		rightHandLength * Mathf.Cos(rightRadians * 1.1f), // Slightly more bent
		rightHandLength * Mathf.Sin(rightRadians * 1.1f)
	);
	
	// Draw left arm with bezier curves
	...
	// Draw the control rods for manipulating hands - animate these too
	...
	private void DrawTiltedKendang(float centerX, float centerY)
{
	...
	if (_drumIsHit)
	{
		float hitOffset = 3.0f * _drumDeformation;
	...
	// Draw only outlines of the drum heads (with animation)
	DrawDrumEnd(smallEnd, smallEndRadius, _lineColor);
	DrawAnimatedDrumHead(bigEnd, bigEndRadius, _lineColor);
	...
	private void DrawAnimatedDrumHead(Vector2 center, float radius, Color color)
	{
		...
		if (_drumIsHit)
		{
			// Calculate deformation effect - waves emanating from hit point
			float deformRadius = radius * 0.5f * (1f + _drumDeformation);
			
			// Direction from center to hit position
			Vector2 hitDir = (_hitPosition - center).Normalized();
			
			// Draw deformation wave circles
			float waveRadius = radius * 0.8f * _drumDeformation;
			DrawCircle(
				center.X + hitDir.X * radius * 0.3f,
				center.Y + hitDir.Y * radius * 0.3f,
				waveRadius, 
				color * new Color(1, 1, 1, _drumDeformation), 
				1.0f
			);
			
			// Secondary smaller wave
			if (_drumDeformation > 0.5f)
			{
				DrawCircle(
					center.X + hitDir.X * radius * 0.2f,
					center.Y + hitDir.Y * radius * 0.2f,
					waveRadius * 0.7f, 
					color * new Color(1, 1, 1, _drumDeformation * 0.8f), 
					0.8f
				);
			}
		}
	...
	private void DrawDrumStick(Vector2 hitPosition, float drumAngle, float animationProgress)
	{
		...
		{
			DrawRect(new Rect2(point, new Vector2(2.5f, 2.5f)), stickColor);
		}
		...
		for (int i = 0; i < 3; i++)
		{
			float fingerAngle = stickAngle + Mathf.Pi/8 * (i - 1);
			Vector2 fingerPos = stickEnd + new Vector2(
				8f * Mathf.Cos(fingerAngle),
				8f * Mathf.Sin(fingerAngle)
			);
			
			DrawSmallCircle(fingerPos, 3.5f, skinColor);
		}
	}
...
private void DrawTruntumElements(float startX, float startY, float width, float height, Color color)
{
	...
	for (int row = 0; row < rows; row++)
	{
		...
		for (int col = 0; col < cols; col++)
		{
			...
			DrawAnimatedTruntumFlower(center, flowerSize, color, _flowerPhases[center]);
		}
	}
	
	// Draw hexagonal connections
	DrawHexagonalConnections(_flowerCenters, color, flowerSpacing);
	
	// Add dots in hexagonal centers
	DrawHexagonalDots(_flowerCenters, startX, startY, width, height, flowerSpacing, rowSpacing, color);
}
...
private void DrawAnimatedBlade(Vector2 origin, float scale, float rotation)
{
	...
	if (_kerisIsShimmering)
	{
		...
		for (int i = 0; i < bladePoints.Length - 1; i++)
		{
			...
			float pointProgress = (float)i / (bladePoints.Length - 1);
			...
			if (Math.Abs(pointProgress - shimmerZone) < 0.25f)
			{
				...
				Vector2 center = TransformPoint(bladePoints[i], origin, scale, rotation);
